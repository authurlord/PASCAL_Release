...
async def run_parallel_evaluation(
    ...
    # Resume support: if output_path exists, skip task_ids already present
    # that have a real (non-infra-error) outcome. infra_err tasks
    # (phase1_passed is None) are re-run. Useful after orchestrator restart.
    done_ids: set = set()
    if Path(output_path).exists():
        try:
            ...
            for _r in _prev.get("results", []):
                if _r.get("phase1_passed") is not None:
                    results.append(_r)
                    total_reward += _r.get("total_reward", 0) or 0
                    if _r.get("phase1_passed"):
                        p1_count += 1
                    if _r.get("phase2_passed"):
                        p2_count += 1
                    completed += 1
                    done_ids.add(_r.get("task_id"))
            if done_ids:
                logger.info("Resume: preserving %d completed tasks from %s",
                            len(done_ids), output_path)
            tasks = [t for t in tasks if t.get("instance_id") not in done_ids]
        except Exception as _e:
            logger.warning("Resume skipped (could not parse %s): %s",
                           output_path, _e)
            results = []
            total_reward = 0.0
            p1_count = 0
    ...
    async def _run_one(i: int, td: dict):
        ...
        async with results_lock:
            ...
            completed += 1
            await _save()
            logger.info("Task %s done (%d/%d) P1=%s reward=%.1f",
                        instance_id, completed, len(tasks),
                        r.get("phase1_passed", False), r.get("total_reward", 0))
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["a-interact", "oracle"],
        default="a-interact",
        help="a-interact runs the PASCAL anchor (default) or the official "
             "ReACT baseline when PASCAL_NO_PROTOCOL=1 is set in the env. "
             "oracle submits ground-truth SQL directly to validate the "
             "evaluation pipeline without invoking the LLM.",
    )
